...
const TxStatus: React.FC<{ status: TxEvent | null }> = ({ status }) => {
  ...
  if (status.type === "signed") {
    statusMessage = `Transaction signed: ${status.txHash.slice(0, 10)}...${status.txHash.slice(-8)}`;
    statusType = "info";
  } else if (status.type === "broadcasted") {
    statusMessage = `Transaction broadcasted: ${status.txHash.slice(0, 10)}...${status.txHash.slice(-8)}`;
    statusType = "info";
  } else if (status.type === "txBestBlocksState") {
    statusMessage = status.found 
      ? `Transaction included in block: ${status.block.hash.slice(0, 10)}...${status.block.hash.slice(-8)}-${status.block.index}`
      : `Transaction broadcasted: ${status.txHash.slice(0, 10)}...${status.txHash.slice(-8)}`;
    statusType = status.found ? "success" : "info";
  } else if (status.type === "finalized") {
    statusMessage = `Transaction finalized in block: ${status.block.hash.slice(0, 10)}...${status.block.hash.slice(-8)}-${status.block.index}`;
    statusType = "success";
  }

  return (
    <div className={`tx-status tx-status-${statusType}`}>
      {statusMessage}
    </div>
  );
};
...
export const Teleport: React.FC = () => {
  ...
  return (
    <Card title="Cross-Chain Teleport" className="transfer-container">
      <div className="transfer-description">
        Teleport your assets instantly between chains with XCM Teleport functionality.
      </div>
      
      <div className="transfer-controls">
        <Button 
          variant="primary"
          onClick={() => teleport("relay")}
          disabled={isLoading || Number(amount) <= 0}
          isLoading={isLoading && txStatus?.type !== "finalized"}
          className="teleport-btn up"
        >
          <span className="teleport-icon">↑</span>
          <span>To Relay Chain</span>
          <span className="teleport-icon">↑</span>
        </Button>
        ...
        <Button 
          variant="primary"
          onClick={() => teleport("para")}
          disabled={isLoading || Number(amount) <= 0}
          isLoading={isLoading && txStatus?.type !== "finalized"}
          className="teleport-btn down"
        >
          <span className="teleport-icon">↓</span>
          <span>To ParaChain</span>
          <span className="teleport-icon">↓</span>
        </Button>
      </div>
